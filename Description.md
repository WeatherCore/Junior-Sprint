...
Junior Sprint 是基于 Skill 框架的企业需求交付训练器，通过客户 → 自我攻坚 → 卡点求助 → 交付验收闭环训练需求理解、问题拆解与自主排错。其含金量在身份与节奏控制：4 种身份语言隔离（客户口吻零技术词、沉默客户零代码、教练严格梯度）、三段式求助判定（目标+卡点+已尝试 才进指导）、渐进梯度 6 级一次只给一级绝不整文件、跨轮读 SPRINT-RETRO.md 主动换档追踪成长曲线、防一键退出要求描述退出理由。工程由 SKILL.md 主入口 + interface.yaml 元数据 + 6 篇 references + 3 个模板构成，训练栈覆盖 Python 与 Java。适合新人补独立交付短板、面试陌生项目快速上手训练。
